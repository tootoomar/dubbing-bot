        ...
        try:
            ...
            for item in parsed:
                if not isinstance(item, dict):
                    continue
                try:
                    st = max(0.0, float(item.get("start", 0)))
...
def extract_timestamped_dubbing_json(audio_path: str, total_duration: float):
    ...
    try:
        offset = 0.0
        chunk_index = 0
        while offset < total_duration - 0.01:
            chunk_start = max(0.0, offset - (overlap if offset > 0 else 0.0))
            chunk_end = min(total_duration, offset + chunk_seconds)
        ...
        for seg in all_segments:
            seg["start"] = max(0.0, min(total_duration, float(seg["start"])))
            seg["end"] = max(seg["start"] + 0.05, min(total_duration, float(seg["end"])))
            text = seg["burmese_text"].strip()
            duplicate = False
